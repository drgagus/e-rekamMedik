...
SESSION_START() ;
				if($_SESSION['posisi'] == "pendaftaran"){
					...
					}elseif ($_SESSION['posisi'] == "dokterumum"  or  $_SESSION['posisi'] == "drgpython" ){
...
<input type="submit" name="poliumum" value="POLI UMUM" Style="background:lime; WIDTH:150; color:blue"; />  
...
<h3><label>DAFTAR ANTRIAN POLI UMUM</label></h3>
...
<?php 
	...
	if(isset($_POST['carinama'])){
		...
		$sql = " select * from ruangtunggupoli WHERE nama like '%".$nama."%' and poli='POLI UMUM' order by noantri " ;
	...
	while($pasien = mysqli_fetch_array($query)){
			...
			echo "<td>" ;
            echo "<a href='polidr.php?nik=".$pasien['nik']."' >POLI UMUM</a>";
			echo "</td>";
            echo "</tr>";
	}		
		...
		$sql = " select * from ruangtunggupoli WHERE norm='$norm' and poli='POLI UMUM' order by noantri " ;
		$query = mysqli_query($db, $sql);
	...
	while($pasien = mysqli_fetch_array($query)){
		...
		$sql = "SELECT * FROM ruangtunggupoli where poli='POLI UMUM' order by noantri";
        $query = mysqli_query($db, $sql);
		while($pasien = mysqli_fetch_array($query)){
			echo "<tr>" ;
			echo "<td><center>".$pasien['noantri']."</center></td>";

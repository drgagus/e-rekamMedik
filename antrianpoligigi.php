...
<h3><label>DAFTAR ANTRIAN POLI GIGI</label></h3>
...
<?php 
	...
	if(isset($_POST['carinama'])){
		...
		$sql = " select * from ruangtunggupoli WHERE nama like '%".$nama."%' and poli='POLI GIGI' order by noantri " ;
	...
	while($pasien = mysqli_fetch_array($query)){
			...
			echo "<td>" ;
            echo "<a href='polidrg.php?nik=".$pasien['nik']."' >POLI GIGI</a>";
			echo "</td>";
            echo "</tr>";
	}		
	...
	while($pasien = mysqli_fetch_array($query)){
		...
		$sql = "SELECT * FROM ruangtunggupoli where poli='POLI GIGI' order by noantri";
        $query = mysqli_query($db, $sql);
		while($pasien = mysqli_fetch_array($query)){
			echo "<tr>" ;
			echo "<td><center>".$pasien['noantri']."</center></td>";

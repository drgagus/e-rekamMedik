...
<h3><label>DAFTAR PASIEN HARI INI</label></h3>
...
<?php 
	...
	if(isset($_POST['carinama'])){
		...
		$sql = " select * from pasienhariini WHERE nama like '%".$nama."%' order by noantri " ;
	...
	while($pasien = mysqli_fetch_array($query)){
			...
			echo "<td>" ;
			echo "<a href='lembarberobat.php?nik=".$pasien['nik']."' target='_blank'>LEMBAR BEROBAT</a>    |    ";
			echo "<a href='lembarrekammedis.php?nik=".$pasien['nik']."' target='_blank'>REKAM MEDIS</a>";
			echo "</td>";
            echo "</tr>";
	}		
		...
		$sql = " select * from pasienhariini WHERE norm='$norm' order by noantri " ;
		$query = mysqli_query($db, $sql);
	...
	while($pasien = mysqli_fetch_array($query)){
		...
		$sql = "SELECT * FROM pasienhariini order by noantri";
        $query = mysqli_query($db, $sql);
		while($pasien = mysqli_fetch_array($query)){
			echo "<tr>" ;
			echo "<td><center>".$pasien['noantri']."</center></td>";
...
<input type="submit" name="hapusdaftar" value="HAPUS DAFTAR" /> 
</fieldset>

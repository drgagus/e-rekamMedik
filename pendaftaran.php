...
<form action="menu.php" method="POST">
<input type="submit" name="inputpasien" value="INPUT PASIEN" Style="WIDTH:110"; /> 
<input type="submit" name="rekapitulasipasien" value="REKAPITULASI" Style="WIDTH:110"; /> 
<input type="submit" name="pasienhariini" value="PASIEN HARI INI" Style="WIDTH:120"; /> 
</form>
...
<form action="pendaftaran.php" method="POST">
	<tr>
	<td><label>Cari Pasien (Nama)</label></td>
	<td>:</td>
...
<form action="pendaftaran.php" method="POST">
	<tr>
	<td><label>Cari Pasien (No. NIK)</label></td>
	<td>:</td>
	<td><input type="text" name="nik" required/></td>
	<td><input type="submit" name="carinik" value="Cari"/></td>
	</tr>
</form>
<form action="pendaftaran.php" method="POST">
	<tr>
	<td><label>Cari Pasien (No. Rekam Medis)</label></td>
	<td>:</td>
	<td><input type="text" name="norm" required/></td>
	<td><input type="submit" name="carinorm" value="Cari"/></td>
	</tr>
</form>
 </table>
...
if(isset($_POST['carinik'])){
	...
	echo "<b>Hasil pencarian NIK: ".$nik."</b>";
}else if (isset($_POST['carinorm'])){
	$norm = mysqli_real_escape_string($db,$_POST['norm']);
	echo "<b>Hasil pencarian no rekam medis: ".$norm."</b>";
}else if (isset($_POST['carinama'])){
	$nama = mysqli_real_escape_string($db,$_POST['nama']);
	echo "<b>Hasil pencarian no rekam medis: ".$nama."</b>";
}else{
}
?>
...
<?php 
	...
	while($pasien = mysqli_fetch_array($query)){
			...
			echo "<td>";
            echo "<a href='datapasienview.php?nik=".$pasien['nik']."'>DATA PASIEN</a>";
            echo "</td>";

            echo "</tr>";
	}		
		
	}else if(isset($_POST['carinik'])){
		$nik = mysqli_real_escape_string($db,$_POST['nik']) ;

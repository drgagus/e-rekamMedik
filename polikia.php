...
<form action="menu.php" method="POST">
<input type="submit" name="antrianpolikia" value="ANTRIAN PASIEN" Style="WIDTH:120"; /> 
<input type="submit" name="rekapitulasipasienkia" value="REKAPITULASI" Style="WIDTH:120"; /> 
</form>
<h4><b>SELAMAT DATANG <?php echo $_SESSION['nama'] ; ?></b></h4>

<table Style="background:lightblue"; >
<form action="polikia.php" method="POST" >
...
<?php 
	...
	if(isset($_POST['carinama'])){
		...
		$sql = " select * from datapasien WHERE nama like '%".$nama."%' AND jeniskelamin='PEREMPUAN' " ;
	...
	while($pasien = mysqli_fetch_array($query)){
			...
			echo "<td>" ;
            echo "<a href='polikeb.php?nik=".$pasien['nik']."' >POLI KIA</a>";
			echo "</td>";
		...
		$sql = " select * from datapasien WHERE norm='$norm' AND jeniskelamin='PEREMPUAN' " ;
		$query = mysqli_query($db, $sql);
	...
	while($pasien = mysqli_fetch_array($query)){
		...
		$sql = "SELECT * FROM datapasien WHERE jeniskelamin='PEREMPUAN' order by norm";
        $query = mysqli_query($db, $sql);
		$no = 1;
        while($pasien = mysqli_fetch_array($query)){
            echo "<tr>";

...
<h3><label>DAFTAR ANTRIAN LABORATORIUM</label></h3>
...
<?php 
	...
	if(isset($_POST['carinama'])){
		...
		$sql = " select * from ruangtunggulab WHERE nama like '%".$nama."%' order by noantri " ;
	...
	while($pasien = mysqli_fetch_array($query)){
			...
			echo "<td>" ;
            echo "<a href='lab.php?nik=".$pasien['nik']."' >LABORATORIUM</a>";
			echo "</td>";
            echo "</tr>";
	}		
		...
		$sql = " select * from ruangtunggulab WHERE norm='$norm' order by noantri " ;
		$query = mysqli_query($db, $sql);
	...
	while($pasien = mysqli_fetch_array($query)){
		...
		$sql = "SELECT * FROM ruangtunggulab order by noantri";
        $query = mysqli_query($db, $sql);
		while($pasien = mysqli_fetch_array($query)){
			echo "<tr>" ;
			echo "<td><center>".$pasien['noantri']."</center></td>";

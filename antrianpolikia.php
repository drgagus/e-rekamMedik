...
<h3><label>DAFTAR ANTRIAN POLI KIA</label></h3>
...
<?php 
	...
	if(isset($_POST['carinama'])){
		...
		$sql = " select * from ruangtunggupoli WHERE nama like '%".$nama."%' and poli='POLI KIA' order by noantri " ;
	...
	while($pasien = mysqli_fetch_array($query)){
		...
		$sql = " select * from ruangtunggupoli WHERE norm='$norm' and poli='POLI KIA' order by noantri " ;
		$query = mysqli_query($db, $sql);
	...
	while($pasien = mysqli_fetch_array($query)){
		...
		$sql = "SELECT * FROM ruangtunggupoli where poli='POLI KIA' order by noantri";
        $query = mysqli_query($db, $sql);
		while($pasien = mysqli_fetch_array($query)){
			echo "<tr>" ;
			echo "<td><center>".$pasien['noantri']."</center></td>";
